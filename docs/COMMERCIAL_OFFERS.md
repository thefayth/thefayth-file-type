# Commercial Offers

These offers are scoped by request. No public fixed pricing is listed.

## Protected File / Provenance Consulting

Best for: creators, builders, and teams preserving original files, proof records, authorship, and disclosure boundaries.

Deliverables may include:

- provenance needs assessment
- file proof and disclosure workflow
- local receipt structure
- public/private data boundary
- TheFAYTH File Type concept walkthrough

## GitHub / Public Surface Packaging

Best for: private products that need public credibility without exposing private implementation.

Deliverables may include:

- public/private boundary review
- README and repo docs
- ownership and license posture
- status, roadmap, FAQ, and launch receipt
- public-safe diagrams and screenshots

## Local AI Workflow Setup

Best for: operators who want AI help with receipts, checkpoints, and owner authority.

Deliverables may include:

- workflow map
- local folder and receipt structure
- task packet templates
- safe automation rules
- operator walkthrough

## AI Brand System Audit

Best for: founders whose public presence feels fragmented across website, GitHub, docs, and visuals.

Deliverables may include:

- brand-fit review
- visual language notes
- rejected-pattern list
- accessibility and trust review
- next-batch creative brief

## AI Website Visual Cleanup

Best for: public sites with generic, repeated, broken, low-trust, or off-brand visuals.

Deliverables may include:

- visual audit
- route-by-route recommendations
- replacement plan
- source receipt review
- publish-ready handoff where supported

## Contact

Portfolio and offer overview: [Faith AI Systems Portfolio](https://thefayth.github.io/faith-ai-systems-portfolio/)
Share / send kit: [Share / Send Kit](https://thefayth.github.io/faith-ai-systems-portfolio/share-send-kit.html)
Work with Faith: [Work With Faith](https://thefayth.github.io/faith-ai-systems-portfolio/work-with-faith.html)
Public prospect packet: [Prospect Packet](https://thefayth.github.io/faith-ai-systems-portfolio/prospect-packet.html)
Sample deliverables: [Sample Deliverables](https://thefayth.github.io/faith-ai-systems-portfolio/sample-deliverables.html)
Buyer FAQ: [Buyer FAQ](https://thefayth.github.io/faith-ai-systems-portfolio/buyer-faq.html)
Pilot sprint menu: [Pilot Sprint Menu](https://thefayth.github.io/faith-ai-systems-portfolio/pilot-sprint-menu.html)
Public proof index: [Public Proof Index](https://thefayth.github.io/faith-ai-systems-portfolio/public-proof-index.html)
Referral and introduction kit: [Referral And Introduction Kit](https://thefayth.github.io/faith-ai-systems-portfolio/referral-introduction-kit.html)
Request scope guide: [Request Scope](https://thefayth.github.io/faith-ai-systems-portfolio/scope-request.html)
Investor or partner brief: [Investor And Partner Brief](https://thefayth.github.io/faith-ai-systems-portfolio/investor-partner-brief.html)

Request a scoped project: [FaithCheltenham.com/contact](https://faithcheltenham.com/contact/)
